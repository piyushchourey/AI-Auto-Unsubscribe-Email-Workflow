...
# Keys the UI may update; order for .env file. Other keys in .env are preserved on save.
_ENV_KNOWN_KEYS = [
    "BREVO_API_KEY",
...
def save_env_file(env_vars):
    """
    Save .env by merging with existing. Only keys in env_vars are updated; all other
    variables already in .env are kept. Adding/removing variables in the app only
    requires updating _ENV_KNOWN_KEYS (for order) and the UI; no need to add new
    f.write() lines here.
    """
    env_path = Path(".env")
    ...
    lines = ["# Unsubscribe Email Workflow Configuration\n", "# Updated by Streamlit UI\n\n"]
    written = set()
    for key in _ENV_KNOWN_KEYS:
        if key in merged:
            lines.append(f"{key}={merged[key]}\n")
            written.add(key)
    others = [k for k in sorted(merged.keys()) if k not in written]
    if others:
        lines.append("\n")
        for key in others:
            lines.append(f"{key}={merged[key]}\n")
...
with tab1:
    ...
    if worker_status:
        ...
        with col1:
            st.info(f"""
            **Status:** {'🟢 Running' if worker_status.get('running') else '🔴 Stopped'}
        ...
        st.info("ℹ️ Worker is disabled. Enable it in the sidebar configuration.")
...
with tab2:
    st.subheader("🧪 Test Intent Detection")
        ...
        if message_text.strip():
            with st.spinner("Analyzing with LLM..."):
                ...
                if status_code == 200:
                    ...
                    if intent:
                        st.success("✅ Unsubscribe Intent Detected!")
                    else:
                        st.info("ℹ️ No Unsubscribe Intent Detected")
...
with tab3:
    ...
    st.warning("⚠️ This will actually blacklist the email in your Brevo account!")
    
    if st.button("🚫 Blacklist Email", type="primary"):
        if test_email:
...
with tab5:
    ...
    st.info("ℹ️ Log viewing feature - Coming soon!")
